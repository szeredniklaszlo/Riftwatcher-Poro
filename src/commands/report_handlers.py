...
from src.constants import MOOD_COMMAND, WEEK_COMMAND
...
async def handle_report_commands(ctx):
    ...
    if content_lower == MOOD_COMMAND.casefold():
        request_id = ctx.create_request_id("mood")
        token = ctx.request_id_context.set(request_id)
        try:
            await ctx.message.delete()
        except (discord.Forbidden, discord.NotFound, discord.HTTPException) as exc:
            ctx.log(f"[mood] Could not delete command message {ctx.message.id}: {exc}")

        try:
            if ctx.mood_request_lock.locked():
                await ctx.message.channel.send("\u23F3 Another report is already in progress. Please wait.")
                return True

            async with ctx.mood_request_lock:
                loading_text = (
                ...
                try:
                    if ctx.db_enabled:
                        snapshot_text = await ctx.mood_service.build_today_win_rate_report()
                        ...
                        ctx.log(f"[mood] Sent stored snapshot report in channel {ctx.daily_channel_id}.")

                        await ctx.mood_service.refresh_recent_matches_snapshot(recent_count=20)
                        refreshed_text = await ctx.mood_service.build_today_win_rate_report()
                        if refreshed_text != displayed_text:
                            await status_message.edit(content=refreshed_text)
                            ctx.log(f"[mood] Updated report after quick refresh in channel {ctx.daily_channel_id}.")
                        else:
                            ctx.log("[mood] Quick refresh produced no visible report change.")
                    else:
                        ...
                        report_text = await ctx.mood_service.build_today_win_rate_report(progress_callback=progress)
                        await status_message.edit(content=report_text)
                        ctx.remember_report_message(status_message)
                        ctx.log(
                            f"[mood] Sent cycle win rate report (since {ctx.report_day_start_hour:02d}:00) "
                    ...
                    ctx.log(f"[mood] Mood report failed: {exc}")
                except Exception as exc:
                    await status_message.edit(content=f"Mood report failed unexpectedly: {exc}")
                    ctx.log(f"[mood] Unexpected mood report failure: {exc}")
        finally:
    ...
    if content_lower == WEEK_COMMAND.casefold():
        ...
        try:
            ...
            async with ctx.mood_request_lock:
                ...
                if status_message.content != loading_text:
                    await status_message.edit(content=loading_text)
                report_text = await ctx.mood_service.build_weekly_win_rate_report(bypass_cache=True)
                await status_message.edit(content=report_text)
